...
import { Sparkles, Loader2, ArrowLeft, Check, Terminal, Copy, AlertTriangle } from "lucide-react"
import {
  Select,
  SelectContent,
  SelectItem,
...
import type { TeamPlan, QueuedTask } from "@/types"
...
export function SmartCreateDialog() {
  ...
  // Load queued tasks when dialog opens
  async function loadQueuedTasks() {
    try {
      ...
      // Non-critical — queue association is optional
    }
  }
        ...
        {step === "goal" && (
          <>
                ...
                <div className="space-y-1.5">
                  ...
                  {taskAlreadyHasTeam && (
                    <div className="flex items-center gap-1.5 rounded-md bg-amber-500/10 px-2 py-1.5 text-xs text-amber-500">
                      ...
                      This task already has team &ldquo;{selectedTask?.teamName}&rdquo; — only one team per task is allowed.
                    </div>
                  )}
                </div>
              )}
            ...
            <DialogFooter>
              ...
              <Button onClick={handleGenerate} disabled={loading || !goal.trim() || taskAlreadyHasTeam} className="gap-1.5">
                {loading ? (
                  <>
